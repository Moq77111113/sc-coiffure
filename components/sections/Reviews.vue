    ...
    <div class="container px-4 md:px-6">
      <div class="flex flex-col items-center space-y-6">
        ...
        <article class="flex flex-col space-y-6">
          <Carousel class="relative mx-auto w-full max-w-[300px] py-6 sm:max-w-[400px] md:max-w-[900px] xl:max-w-full"
            :opts="{
              align: 'center',
              loop,
            ...
            <CarouselContent>
              <CarouselItem v-for="(
                ...
                ) in reviews" :key="index" class="md:basis-1/3 xl:basis-1/4">
                <article
                  ...
                  <div class="flex flex-col items-center">
                    ...
                    <div class="flex py-2">
                      <Icons.Star v-for="i in 5" :id="`${owner.replace(/\W/gi, '')}-star-${i}`" :key="i"
                        fill-color="hsl(48, 100%, 48%)" :fill-percentage="getFillPercentage(score, i)" />
                    </div>
                  </div>

                  <p class="prose-invert line-clamp-2 text-sm md:line-clamp-3 lg:line-clamp-4">
                    {{ content }}
                  </p>
                </article>
              </CarouselItem>
