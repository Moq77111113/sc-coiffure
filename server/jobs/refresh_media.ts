...
const buildUrl = (base: string, token: string, processCount?: number): string => {
...
const extractAndSaveMedia = async (picture: Media['data'][number]) => {
  ...
  if (!oe) {
    console.error(`Could not find the outlink expiration time for ${picture.id}`)
    return
  }
...
export default defineCronHandler(
  ...
  async () => {
    ...
    try {
      ...
      console.error(err)
      console.warn('Refresh media failed')
    }
  },
