...
<template>
  <section id="contact" v-motion-slide-visible-once-top class="w-full py-12 md:py-24">
    <div class="container px-4 md:px-6">
      <article class="flex flex-col items-center justify-center space-y-6 text-center">
        ...
        <form class="flex flex-col items-center space-y-4 w-full" @submit.prevent="submit">
          <label for="contact" class="sr-only">Votre email ou numéro de téléphone</label>
          <Input
v-model="form.contact" type="text" autocomplete="email"
            placeholder="Votre email ou numéro de téléphone" name="contact" class="max-w-sm placeholder:text-muted" />
          <label for="message" class="sr-only">Votre message</label>
          <Textarea
v-model="form.message" autocomplete="off" placeholder="Votre message" name="message"
            class="max-w-2xl placeholder:text-muted h-52 my-3 rounded-lg borderBlack p-4" />
          <Input v-model="form.honey" class="hidden" type="text" name="scs-honey" />
